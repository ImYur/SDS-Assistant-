...
def chat(messages, model="gpt-4o-mini", temperature=0.7, max_tokens=350):
    """
    Lightweight wrapper over OpenAI Chat Completions HTTP API.
...
def gen_reply(profile, history_messages, system_map):
    """
    history_messages: [{role:'user'|'assistant', content:'...'}, ...] (останні 12 ходів достатньо)
    """
    system = system_map.get(profile, system_map["Yurii"])
...
def assistant_answer(kb_text, question):
    """
    Відповіді для внутрішнього 'ChatGPT Assistant' топіку: пошук по KB-снапшоту.
    """
    messages = [
        {"role": "system", "content":
